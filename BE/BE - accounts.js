...
const TEST_ACCOUNT_NAME = "BE - Example Account"; // Specify which account to test with
...
const rebuildCurrentAccount = () => {
  ...
  // --- Set row height ---
  const lastUpdateRow = targetSheet.getLastRow() + 2;
  const totalRows = lastUpdateRow;
  Array.from({ length: totalRows }, (_, i) => i + 1).forEach(r =>
    targetSheet.setRowHeight(r, 21)
  );
...
const quickUpdateAccounts = () => {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const masterSheet = ss.getSheetByName("BE - Master Ledger");
  if (!masterSheet) throw new Error("❌ 'BE - Master Ledger' sheet not found.");
  ...
  accountsToProcess.forEach(account => {
    ...
    if (sheet) {
      ...
      // --- Set row height for timestamp row ---
      sheet.setRowHeight(lastUpdateRow, 21);
    }
  });
...
const smartUpdateAccounts = () => {
  ...
  const headers = data[0];
  const accountCol = headers.indexOf("Account");
  const transactionCol = headers.indexOf("BE-Transaction Number");
  const debitCol = headers.indexOf("Debit (VND)");
  const creditCol = headers.indexOf("Credit (VND)");
